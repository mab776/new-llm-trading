...
class Portfolio:
    ...
    def _calculate_position_size(
        ...
        max_margin_pct mirrors live position_sizing.max_position_pct: the margin
        committed to one trade never exceeds this FRACTION of the balance
        (None = uncapped). Because both terms scale with the same balance the
        cap folds into the risk fraction and stays scale-invariant.
        """
        effective_risk = risk_pct
        if max_margin_pct is not None:
            effective_risk = min(effective_risk, max_margin_pct)
        margin = self.balance * effective_risk  # margin allocated to this trade
    ...
    def open_trade(
        ...
        size = self._calculate_position_size(
            entry_price, leverage, risk_pct, max_margin_pct
        )
