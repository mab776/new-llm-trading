...
| `live_reconcile.py` | **"Does the sim reproduce live?"** Runs the deployed configs over the live window (maker=expected + taker=counterfactual) and prints them beside the **actual** result parsed from `logs/decisions-*.jsonl`. Compares deposit-independent **trading P/L** (not balance), auto-flags deposits/config-changes, and nudges the warmup start off Bitget's ragged 1h edge. `python -m opt.live_reconcile [--start ... --end ... --auto-initial]`. |
| `probe_reserved.py` | Shared pot vs reserved per-asset thirds (2026-07-20): **keep shared** — reserved trades +70% more but grows less (4.79× vs 5.88× structural; 3.89× vs 5.65× @$193+mins); cross-subsidy compounding is the edge. Results in `probe_reserved_results.txt`. |
| `probe_rotation.py` | Cross-asset rotation (evict weakest other-symbol position for a cap-squeezed STRONG entry; first **multi-asset TRAIN/TEST fold harness**): **REJECTED for growth** — TRAIN winner failed TEST (noise). Post-hoc: improves worst-folds (robustness idea, see `GOOD_IDEAS.md` #4). Knobs `rotate_*` in `simulate_multi`, default-off engine-identical. |
| `probe_overshoot.py` | Conditional cap-overshoot / min-size rescue (floor a MIN_SIZE_SKIPped entry to the exchange min iff \|score\|≥S within caps×(1+O)): **all gates passed** — rescue>skip split-consistent (holdout 5.65→6.01×), O inert, S-vs-floor line = noise. Small-account provision, NOT deployed (needs scheduler code). Knobs `min_size_overshoot*`, floor control arm. `GOOD_IDEAS.md` #2. |
